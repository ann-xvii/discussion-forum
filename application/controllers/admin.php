...
class Admin extends MY_Controller {
	...
	public function login() {
		// the first thing the login does is define form validation rules for the usr_email and usr_pwd form elements
		// these will govern how the data submitted by the user in the admin/login.php view file is validated
		...
		// test to see whether form has been submitted
		if ($this->form_validation->run() == FALSE) {
			// if form hasn't been submitted, we'll load the view files to display the login form and wait for a response from the user
			...
			if ($query->num_rows() == 1) { // One matching row found
				foreach ($query->result() as $row) {
					...
					if ($hash != $row->usr_hash) {
						...
						// above we compared the hash with the hash stored in the database belonging to the user
						// if it does match, then the user must have typed the correct password; so we log them in by creating 
						// a CodeIgniter session for them
						$data = array(
							'usr_id' => $row->usr_id,
