...
class Comments extends MY_Controller {
	...
	public function index() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			// assuming that there were no errors with the form and it is submitted without the need to report anything requiring the user's attention
			// the index() function attempts to write the comment to the comments table in the database
			// before we do that, we need to package all our data into an array that will be passed to Comments_model

			if ($this->Comments_model->new_comment($data)) {
				// if the insertion was successful, we redirect the user to the comments controller's index function and pass $ds_id as the third
				// parameter where the index() function will begin, displaying all active comments associated with the selected discussion
	...
	// the flag function will enable the user to indicate that a comment requires moderation by an admin
	// stepping back to the discussions controller, the discussions controller will display a list of active discussions
